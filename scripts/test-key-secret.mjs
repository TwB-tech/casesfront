const projectId = '69e8bc1500162d3defdb';
const apiKey = 'standard_';
const email = 'freshuser001@example.com';
const password = 'TestPass123!';
const endpoint = 'https://tor.cloud.appwrite.io/v1';

async function main() {
  // Create session via direct (without API key? but we'll use the secret)
  const loginRes = await fetch(`${endpoint}/account/sessions/email`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-Appwrite-Project': projectId },
    body: JSON.stringify({ email, password })
  });
  const loginData = await loginRes.json();
  const secret = loginData.secret;
  console.log('Secret length:', secret.length);

  // GET /account with BOTH API key and session secret
  const bothRes = await fetch(`${endpoint}/account`, {
    method: 'GET',
    headers: {
      'X-Appwrite-Project': projectId,
      'X-Appwrite-Key': apiKey,
      'X-Appwrite-Session': secret
    },
    signal: AbortSignal.timeout(15000)
  });
  console.log('Both key+secret status:', bothRes.status);
  const bothData = await bothRes.json();
  console.log('Both data:', JSON.stringify(bothData).substring(0, 200));
}
main().catch(console.error);
