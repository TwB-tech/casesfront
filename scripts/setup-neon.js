...
async function ensureUsersTable() {
  ...
  await execute('CREATE INDEX IF NOT EXISTS idx_users_verification_token ON users(verification_token)');
  await execute('CREATE INDEX IF NOT EXISTS idx_users_organization_id ON users(organization_id)');
...
async function seedTestUsers() {
  ...
  const stmt = `INSERT INTO users (id, email, username, password, role, status, email_verified, created_at, updated_at)
    VALUES ($1, $2, $3, $4, $5, 'Active', $6, NOW(), NOW())
    ON CONFLICT (email) DO NOTHING`;

  for (const user of users) {
    await execute(stmt, [user.id, user.email, user.username, user.password, user.role, user.email_verified]);
...
async function main() {
  ...
  log.info(`Target: ${DATABASE_URL.replace(/\/\/.*@/, '//[REDACTED]@')}`);
