/**
 * Production Smoke Test (Corrected)
 *
...
async function createUser(email, password, username, role = 'individual') {
  // 1. Create Appwrite account (no session required, uses API key)
  ...
  // 2. Create session (no API key needed, but we can include it; it's harmless)
  // We'll use the same endpoint the frontend uses (through proxy)
  const { data: session } = await api('POST', '/account/sessions/email', { email, password });
  if (!session?.secret) {
    throw new Error('Session creation succeeded but no secret returned');
  }
...
async function login(email, password) {
  const { data: session } = await api('POST', '/account/sessions/email', { email, password });
  if (!session?.secret) {
    throw new Error('Login succeeded but no secret returned');
  }
  setSession(session.secret);
...
async function createCase(title) {
  ...
  await api('POST', '/case', {
    ...
    organization_id: null,
    created_by: 'current', // use authenticated user
  });
  return caseId;
}
