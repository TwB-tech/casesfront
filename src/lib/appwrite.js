...
export const db = {
  // Normalize Appwrite document to Supabase-like shape
   ...
   async get(collection, docId) {
    ...
    // If provided and a conflict occurs, we surface the error (caller must resolve).
    async create(collection, data, documentId) {
      const autoGenerated = documentId === undefined;
      ...
      try {
        ...
        if (isConflict && autoGenerated) {
          // Only auto-retry when we generated the ID ourselves
          const newId = ID.unique();
          return this.create(collection, data, newId);
        }
