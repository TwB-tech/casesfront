...
const CardDescription = React.forwardRef(({ className, ...props }, ref) => {
  ...
  return (
    <p
      ref={ref}
      className={cn('text-sm', isFuturistic ? 'text-aurora-muted' : 'text-neutral-500', className)}
      {...props}
    />
  );
});
...
const StatCard = React.forwardRef(
  ({ className, icon: Icon, title, value, trend, trendUp = true, ...props }, ref) => {
    const { isFuturistic } = useTheme();

    return (
      <Card ref={ref} hover className={cn('relative overflow-hidden', className)} {...props}>
        {isFuturistic && (
        ...
        <div className="flex items-start justify-between">
          ...
          {Icon && (
            ...
            >
              <Icon
                className={cn('text-xl', isFuturistic ? 'text-aurora-primary' : 'text-primary-600')}
              />
            </div>
          )}
        </div>
