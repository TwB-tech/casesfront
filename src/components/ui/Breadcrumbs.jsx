...
const Breadcrumbs = ({ customRoutes }) => {
  ...
  const routes = customRoutes || pathSnippets.map((snippet, index) => {
  ...
  return (
    <div className={`breadcrumbs-container ${isFuturistic ? 'dark' : ''}`}>
      <Breadcrumb
        separator={
          <RightOutlined className={`text-xs ${isFuturistic ? 'text-aurora-muted' : 'text-neutral-400'}`} />
        }
        items={[
          {
            title: (
              <Link to="/home" className="flex items-center gap-1">
                ...
                <span className={isFuturistic ? 'text-aurora-muted hover:text-aurora-text' : 'text-neutral-500 hover:text-neutral-700'}>
                  Home
                </span>
              </Link>
            ),
