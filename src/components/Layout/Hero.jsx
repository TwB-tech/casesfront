        ...
        <div className="flex-1 text-center md:text-left space-y-6 md:space-y-8">
          {isFuturistic && (
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-aurora-primary/10 border border-aurora-primary/30 mb-4">
              ...
              <span className="text-sm font-medium text-aurora-muted">
          ...
          <div className="space-y-4">
            {[
              {
                title: 'End burnout cycles',
                desc: 'Get instant access to verified law firms and advocates when caseloads spike',
              },
              {
                title: 'Cut admin time by 60%',
                desc: 'AI-powered case management that handles documentation automatically',
              },
              {
                title: 'Never miss a deadline',
                desc: 'Your AI assistant Reya monitors cases 24/7 and proactively alerts you',
              },
            ].map((item, index) => (
              <div key={index} className="flex items-start gap-4 group">
                ...
                >
                  <svg
                    className={`w-4 h-4 ${isFuturistic ? 'text-white' : 'text-success-600'}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <div>
                  <p className={isFuturistic ? 'text-aurora-text' : 'text-neutral-700'}>
                    <strong className={isFuturistic ? 'text-aurora-primary' : 'text-primary-800'}>
                      {item.title}
                    </strong>
