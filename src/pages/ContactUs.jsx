  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <Row gutter={[48, 48]}>
        ...
        <Col xs={24} md={12}>
          <Card
            className={`h-full ${isFuturistic ? 'bg-cyber-card border-cyber-border' : 'shadow-sm'}`}
        ...
        <Col xs={24} md={12}>
          <Card
            className={`h-full ${
              isFuturistic ? 'bg-cyber-card border border-cyber-border' : 'shadow-sm'
            }`}
            style={{ borderRadius: '16px' }}
            styles={{ body: { padding: '32px' } }}
          >
            ...
            <div className="space-y-8">
              {contactInfo.map((item, idx) => (
                <div key={idx} className="flex gap-4">
                  ...
                  >
                    <item.icon
                      className={`text-xl ${isFuturistic ? 'text-aurora-primary' : 'text-primary-600'}`}
                    />
                  </div>
                  <div>
                    <h4
            ...
            {/* Development Credit */}
            <div
              className={`mt-8 pt-8 border-t ${isFuturistic ? 'border-cyber-border' : 'border-neutral-200'}`}
            >
            ...
            >
              <Text
                className={`text-sm mb-4 block ${isFuturistic ? 'text-aurora-muted' : 'text-neutral-500'}`}
              >
                Follow Us
              </Text>
      ...
      <div
        className={`mt-12 rounded-2xl overflow-hidden ${
          isFuturistic ? 'bg-cyber-card border border-cyber-border' : 'bg-neutral-100'
        }`}
      >
        <div
          className={`h-64 flex items-center justify-center ${
            isFuturistic
              ? 'bg-gradient-to-br from-cyber-surface to-cyber-bg'
              : 'bg-gradient-to-br from-primary-50 to-neutral-100'
          }`}
        >
          <div className="text-center">
            <EnvironmentOutlined
              className={`text-5xl mb-4 ${isFuturistic ? 'text-aurora-muted' : 'text-neutral-400'}`}
            />
            <Text className={`text-lg ${isFuturistic ? 'text-aurora-muted' : 'text-neutral-500'}`}>
              Nairobi, Kenya
            </Text>
            <Text
              className={`block text-sm mt-2 ${isFuturistic ? 'text-aurora-muted/70' : 'text-neutral-400'}`}
            >
