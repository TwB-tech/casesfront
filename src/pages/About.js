...
const About = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {/* Stats Section */}
      <div
        className={`grid grid-cols-2 md:grid-cols-4 gap-4 mb-16 p-8 rounded-2xl ${
          isFuturistic
      ...
      <Row gutter={[48, 48]} style={{ marginBottom: '80px' }} align="middle">
        <Col xs={24} md={12}>
          <div>
            ...
            >
              WakiliWorld was created by lawyers who understood a fundamental truth: the
              administrative burden of modern practice is killing what made us go into law in the
              first place.
            </p>
            <p
            ...
            >
              ...
              automation, on-demand law firm and advocate support, and tools built exclusively for
              legal professionals.
            </p>
          </div>
        </Col>
        <Col xs={24} md={12}>
          ...
          >
            <RocketOutlined
              className={`text-6xl mb-4 ${isFuturistic ? 'text-aurora-primary' : 'text-white'}`}
            />
            <h3
              className={`text-5xl font-bold mb-2 ${isFuturistic ? 'text-aurora-text' : 'text-white'}`}
            >
              10,000+
            </h3>
          ...
          {values.map((value, idx) => (
            <Col xs={24} md={8} key={idx}>
              ...
              >
                ...
                >
                  <value.icon
                    className={`text-3xl ${isFuturistic ? 'text-aurora-primary' : 'text-primary-600'}`}
                  />
                </div>
                <h3
                  className={`text-xl font-bold mb-4 ${isFuturistic ? 'text-aurora-text' : 'text-primary-900'}`}
                >
                  {value.title}
                </h3>
          ...
          {[
            {
              icon: RocketOutlined,
              title: 'AI-Powered',
              desc: 'Intelligent automation that handles routine tasks',
            },
            {
              icon: SafetyOutlined,
              title: 'Secure',
              desc: 'Bank-grade encryption and compliance features',
            },
            {
              icon: TeamOutlined,
              title: 'Scalable Support',
              desc: 'On-demand law firms and advocates when you need them',
            },
            {
              icon: HeartOutlined,
              title: 'Built by Lawyers',
              desc: 'Created by legal professionals who understand your needs',
            },
          ].map((feature, idx) => (
        ...
        <Row gutter={[24, 24]}>
          {teamMembers.map((member, index) => (
            <Col xs={12} sm={8} md={4} key={index}>
              ...
              >
                <div
                  className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center text-xl font-bold ${
                    isFuturistic
                ...
                <h3
                  className={`font-semibold mb-1 ${isFuturistic ? 'text-aurora-text' : 'text-primary-900'}`}
                >
                  {member.name}
                </h3>
                <p
                  className={`text-sm mb-1 ${isFuturistic ? 'text-aurora-primary' : 'text-primary-600'}`}
                >
                  {member.role}
                </p>
      ...
      >
        <h2
          className={`text-3xl font-bold mb-4 ${isFuturistic ? 'text-aurora-text' : 'text-white'}`}
        >
